...
|-------------------|---------------------|--------------|
| OpenAI/Anthropic APIs | Ollama (local) + OpenRouter (free tier) | Learn LLM integration basics with dual options |
...
**LLM Usage (Dual-Option Architecture):**

**Option 1: Local (Primary)**
...
- **Fallback**: Always available, no internet needed

**Option 2: OpenRouter API (Secondary)**
- **Models** (Free tier - choose based on your needs):
  - `mistralai/mistral-small-3.1-24b-instruct:free` (24B - Recommended: good balance)
  - `meta-llama/llama-3.3-70b-instruct:free` (70B - Higher quality, slower)
  - `openai/gpt-oss-20b:free` (20B - OpenAI architecture)
  - `openai/gpt-oss-120b:free` (120B - Largest, most capable)
  - `deepseek/deepseek-r1-0528:free` (DeepSeek reasoning model)
  - `z-ai/glm-4.5-air:free` (GLM architecture)
- **Purpose**: Higher quality suggestions when API available
- **Cost**: $0 using free tier models
- **Requirement**: API key + internet connection
- **Use case**: Better performance for complex rewrites
...
| **LLM (Local)** | Ollama + Ministral-3B | Primary content analysis | Free |
| **LLM (API)** | OpenRouter API | Secondary (free tier) | Free |
...
1. **Set up free development environment**
   ...
   - Create conda environment: `conda create -n ats-resume python=3.10`
   - Activate: `conda activate ats-resume`
...
3. **LLM**: Dual setup
   - Primary: Ollama with Ministral-3B-Q4_K_M (local, GPU-efficient)
   - Secondary: OpenRouter with free models (API-based, better quality)
